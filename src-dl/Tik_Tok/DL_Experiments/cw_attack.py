...
from keras.layers import Conv1D, MaxPooling1D, BatchNormalization, \
    Dense, Activation, Dropout, Flatten, ELU
...
def attack(X_train, y_train, X_valid, y_valid, X_test, y_test, args, VERBOSE=1):
    # Map labels to 0-based contiguous indices
    all_labels = np.concatenate((y_train, y_valid, y_test))
    unique_labels, mapped_all = np.unique(all_labels, return_inverse=True)
    num_classes = len(unique_labels)

    # Split back into train/val/test
    y_train = mapped_all[:len(y_train)]
    y_valid = mapped_all[len(y_train):len(y_train)+len(y_valid)]
    y_test = mapped_all[len(y_train)+len(y_valid):]
    ...
    model = ConvNet.build(classes=num_classes, input_shape=(5000, 1))
